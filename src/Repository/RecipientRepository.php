...
class RecipientRepository extends ServiceEntityRepository
{
    ...
    public function findRecipientsSending(string $recipientStatus, string $mailStatus, int $limit): array
    {
        return $this->createQueryBuilder('r')
            ...
            ->where('(m.sent_at_date is null OR m.sent_at_date < now()) AND r.status = :status_r AND m.status = :status_m')
            ->orWhere('(m.sent_at_date is null OR m.sent_at_date < now()) AND r.status = :status_r AND cc.status = :status_m')
